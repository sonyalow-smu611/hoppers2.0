...
function InfinityTrack({
  ...
  const glyphs = INFINITY_TRACK_BLOCKS.map((_, index) => blocks[index] ?? INFINITY_TRACK_BLOCKS[index]);
      ...
      <span
        ...
        <span
          ...
          {gridCells.map((glyph, index) => (
            <span key={index} className="flex h-[1ch] w-[1ch] items-center justify-center">
              {glyph}
            </span>
          ))}
        </span>
